	...
	private function Add() {
		...
		if(!Session::Logging()) {
            ...
            if(Protections::Permission('permission_admin_access') == 'access' AND Protections::Permission('permission_admin_events') == 'access') {
                ...
                if(!empty($this->title) && !empty($this->content) && !empty($this->description)) {
                    ...
                    if(isset($_FILES['file']) AND !empty($_FILES['file']['name'])) {
                        ...
                        if($_FILES['file']['size'] <= $tailleMax) {
                            ...
                            if(in_array($extensionUpload, $extensionsValides)) {
                                ...
                                if($resultat) {
                                    $Insert = $this->db->prepare('INSERT INTO news(user_id, title, content, description, date, image) VALUES(?, ?, ?, ?, ?, ?)');
                                    $Insert->execute(array(Session::Account('id'), $this->title, $this->content, $this->description, date('Y-m-d H:i:s'), '/public/img/uploads/'.$namee.'.'.$extensionUpload));
                                    $status = 'success';
                                    $response = 'Votre actualité a bien été publié';
                                } else {
                                    $status = 'error';
