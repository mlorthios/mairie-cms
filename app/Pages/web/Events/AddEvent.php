...
namespace App\Pages\web\Events;
    ...
    private function Add() {
        ...
        if(!empty($this->first_name) && !empty($this->message)) {
            ...
            if($responseData->success) {
                $r = Database::PDO()->prepare('SELECT * FROM events_comments WHERE ip = ? AND event_url = ?');
                $r->execute(array(Functions::AddressIP(), $this->url));

                $row = $r->rowCount();

                if($row == 0) {
                    $check = Database::PDO()->prepare('SELECT * FROM events WHERE url = ?');
                    ...
                    if($row > 0) {
                        if(iconv_strlen($this->first_name) >= 3) {
                            if(iconv_strlen($this->message) >= 30) {
                                ...
                                $insert = Database::PDO()->prepare('INSERT INTO events_comments(event_url, first_name, content, date, ip) VALUES(?, ?, ?, ?, ?)');
                        ...
                        $response = 'Une erreur est survenue : ' . $this->url;
                        $status = 'error';
                    }
                ...
                $response = 'Veuillez réessayer, nous vous avons détecté comme ROBOT';
                $status = 'error';
            }
        } else {
